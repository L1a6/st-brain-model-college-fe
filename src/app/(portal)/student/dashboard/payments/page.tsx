  ...
  return (
    <div className="min-h-screen bg-white p-6 md:p-8 lg:p-10">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="mb-3 text-xs uppercase tracking-[0.3em] text-slate-500">Student Payments</p>
          <h1 className="font-display text-3xl font-semibold text-[#0A1F44] md:text-5xl">Payments by category</h1>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <p className="mb-1 text-xs uppercase tracking-[0.2em] text-slate-500">Summary</p>
            ...
            <section key={item.category} className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_14px_36px_rgba(15,23,42,0.06)] transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(10,31,68,0.12)]">
              <div className="relative min-h-52 md:min-h-60">
                ...
                <div className="absolute bottom-5 right-5 text-right text-white">
                  <div className="text-3xl font-bold md:text-4xl">₦{item.amount.toLocaleString()}</div>
                  <div className="mt-1 text-xs uppercase tracking-[0.24em] text-white/75">{item.info.label}</div>
                </div>
              </div>
      ...
      <div className="mt-6 flex flex-wrap gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <label className="flex flex-1 min-w-[220px] flex-col gap-2 text-sm text-slate-600">
          Term
          <input
            value={term}
            onChange={(event) => setTerm(event.target.value)}
            className="rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition-colors focus:border-[#0A1F44]"
          />
        </label>
        <label className="flex flex-1 min-w-[220px] flex-col gap-2 text-sm text-slate-600">
          Academic year
