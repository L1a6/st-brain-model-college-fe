...
const TeacherQuizzesPage = () => {
  ...
  const teacherId =
    (user as { teacher_id?: string; id?: string } | null)?.teacher_id ??
  ...
  const teacherQuizzes = useMemo<QuizItem[]>(
    () => getQuizItems(quizzesQuery.data),
    [quizzesQuery.data]
  )

  const publishedCount = teacherQuizzes.filter(
    (quiz) => (quiz?.status || "published").toLowerCase() === "published"
  ).length
  const draftCount = teacherQuizzes.filter(
    (quiz) => (quiz?.status || "published").toLowerCase() === "draft"
  ).length
  ...
  const addQuestion = () =>
    setQuestions((currentQuestions) => [...currentQuestions, emptyQ()])
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm text-slate-600 md:text-base">
            Build assessments that land directly in your students&apos; dashboard once
            published.
          </p>
        </div>
    ...
    <div className="min-h-screen bg-white p-6 md:p-8 lg:p-10">
      ...
      <div className="mb-6 overflow-hidden rounded-3xl bg-linear-to-br from-[#0A1F44] via-[#12396D] to-[#0E234D] text-white shadow-[0_24px_70px_rgba(10,31,68,0.18)]">
        <div className="grid gap-8 p-6 md:p-8 lg:grid-cols-[1.2fr_0.8fr] lg:p-10">
          <div>
            <div className="mb-4 text-xs tracking-[0.3em] text-white/70 uppercase">
              Quiz studio
            </div>
            <h2 className="font-display mb-4 text-3xl leading-tight font-light md:text-5xl">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-white/80 md:text-base">
              Use the teacher dashboard to build quizzes, assign them to a class, and
              publish them so they appear in the student dashboard automatically.
            </p>
          </div>
        ...
        <>
          <div className="mb-6 grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm transition-colors outline-none focus:border-[#0A1F44]"
              />
            </div>
            <div>
              <label className="mb-2 block text-[11px] font-semibold tracking-[0.18em] text-slate-500 uppercase">
            ...
            <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600 xl:col-span-4">
              <span>
                Assigned class:{" "}
                <span className="font-semibold text-[#0A1F44]">{selectedClassLabel}</span>
              </span>
              <span>
                Students in class:{" "}
                <span className="font-semibold text-[#0A1F44]">
                  {classes.find((item) => item?.id === selectedClass)?.students?.length ??
                    0}
                </span>
              </span>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="font-display text-2xl font-semibold text-[#0A1F44]">
                Questions
              </h3>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">
                        Set the prompt, correct answer, and points.
                      </div>
                    </div>
                  </div>
                  {questions.length > 1 && (
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          updateQuestion(
                            index,
                            "type",
                            event.target.value as QForm["type"]
                          )
                        }
                        className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm transition-colors outline-none focus:border-[#0A1F44]"
                      >
                  ...
                  {question.type === "multiple_choice" && (
                    <div className="mb-4">
                      ...
                      <p className="mt-2 text-xs text-slate-500">
                        Mark the radio circle for the correct answer.
                      </p>
                    </div>
                  )}
          ...
          <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h4 className="mb-4 text-sm font-extrabold tracking-[0.18em] text-[#0A1F44] uppercase">
              Assigned students (
              {classes.find((item) => item.id === selectedClass)?.students?.length ?? 0})
            </h4>
          ...
          {quizzesQuery.isLoading ? (
            ...
            teacherQuizzes.map((quiz) => {
              ...
              return (
                <div
                  key={quiz.id}
                  className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_14px_36px_rgba(15,23,42,0.06)]"
                >
                  <div className="relative min-h-52">
                    ...
                    <div className="absolute right-5 bottom-5 text-right text-white">
                      <div className="text-3xl font-bold">
                        {quiz.timeLimitMinutes ?? 30}m
                      </div>
                      <div className="mt-1 text-xs tracking-[0.22em] text-white/75 uppercase">
                        Time limit
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <h4 className="font-display mb-2 text-2xl font-semibold text-[#0A1F44]">
                          {quiz.title}
                        </h4>
                        <p className="text-sm text-slate-600">
                          {quiz.description || quiz.subject}
                        </p>
                      </div>
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-right">
                        <div className="mb-1 text-[11px] tracking-[0.18em] text-slate-500 uppercase">
                          Due
                        </div>
                        <div className="text-sm font-semibold text-[#0A1F44]">
                    ...
                    <div className="mb-5 flex flex-wrap gap-2">
                      ...
                      <Badge variant="secondary">
                        {normalizeArray(quiz.questions).length || 0} items
                      </Badge>
                      <Badge variant="outline">
                        Class: {quiz.classId ?? quiz.class_id ?? "-"}
                      </Badge>
                    </div>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            const submissions =
                              (response as { data?: { submissions?: SubmissionItem[] } })
                                .data?.submissions ?? []
                            const averageScore = submissions.length
                              ? Math.round(
                                  submissions.reduce(
                                    (sum: number, submission) =>
